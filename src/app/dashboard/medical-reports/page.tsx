              ...
              <div className="space-y-3">
                {reports.map((report) => (
                  <div
                    key={report.id}
                    onClick={() => {
                      window.location.href = `/dashboard/medical-reports/${report.id}`;
                    }}
                    className="cursor-pointer rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition hover:border-teal-500 hover:shadow-md"
                  >
...
{(() => {
  const status = report.report_analysis?.[0]?.status;
